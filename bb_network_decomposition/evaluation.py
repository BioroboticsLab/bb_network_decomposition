...
def evaluate_network_factors(df, model=None, n_splits=25, groupby=None, labels=location_labels,
                             factors=default_factors, scoring=None):
...
def evaluate_bee_subsample(keep_proportion, interactions, alive_matrices, bee_ids, bee_ages, alive_df,
                           from_date, num_factors_per_mode, location_dataframe, evaluation_kws={}):
    num_days = interactions.shape[0]
    num_entities = int(interactions.shape[1] * keep_proportion)
    keep_entities = sorted(np.random.choice(
        range(interactions.shape[1]),
        size=num_entities,
        replace=False))
